#!/usr/bin/env node
// class 渲染测试器。 node scripts/test-class.mjs [case] [style|all]  |  node scripts/test-class.mjs all <style>
import fs from 'node:fs'; import path from 'node:path'; import { execFileSync } from 'node:child_process';
const SCRIPTS=path.dirname(new URL(import.meta.url).pathname), ROOT=path.resolve(SCRIPTS,'..');
const CASES=path.join(ROOT,'examples/class/cases'), OUT=path.join(ROOT,'examples/class/out');
const RENDER=path.join(SCRIPTS,'render-class.mjs'), SVG=path.join(SCRIPTS,'svg-export.mjs');
