#!/usr/bin/env node
// state 渲染测试器 —— 挑 case / 挑风格快速验证 render-state。
//   node scripts/test-state.mjs            # 列 case + 风格
//   node scripts/test-state.mjs <case> [style|all]   |   node scripts/test-state.mjs all <style>
import fs from 'node:fs'; import path from 'node:path'; import { execFileSync } from 'node:child_process';
const SCRIPTS=path.dirname(new URL(import.meta.url).pathname), ROOT=path.resolve(SCRIPTS,'..');
const CASES=path.join(ROOT,'examples/state/cases'), OUT=path.join(ROOT,'examples/state/out');
const RENDER=path.join(SCRIPTS,'render-state.mjs'), SVG=path.join(SCRIPTS,'svg-export.mjs');
