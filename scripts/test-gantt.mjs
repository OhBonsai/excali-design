#!/usr/bin/env node
// gantt 渲染测试器。 node scripts/test-gantt.mjs [case] [style|all] | node scripts/test-gantt.mjs all <style>
import fs from 'node:fs'; import path from 'node:path'; import { execFileSync } from 'node:child_process';
const SCRIPTS=path.dirname(new URL(import.meta.url).pathname), ROOT=path.resolve(SCRIPTS,'..');
const CASES=path.join(ROOT,'examples/gantt/cases'), OUT=path.join(ROOT,'examples/gantt/out');
const RENDER=path.join(SCRIPTS,'render-gantt.mjs'), SVG=path.join(SCRIPTS,'svg-export.mjs');
...
const montage=(p,o,t)=>{if(!HASM||p.length<2)return;try{execFileSync('montage',[...p,'-tile',t,'-geometry','680x+8+8','-background','#ccc',o]);console.log('  ▦',path.relative(ROOT,o));}catch{}};
...
if(a1!=='all'&&a2==='all')montage(made,path.join(OUT,`${a1}.ALL-styles.png`),`1x${made.length}`);
else if(a1==='all'&&a2!=='all')montage(made,path.join(OUT,`ALL-cases.${a2}.png`),'1x');
