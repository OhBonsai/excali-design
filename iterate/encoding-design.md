# encode.ir 这层的理论背景(为什么这么选通道)

> 这是 `references/encoding.md`(提示词)背后的**研究/依据**记录,不在运行时读。
> 提示词只给「怎么做」,本文件留「为什么」。理论:Bertin 视觉变量 / Cleveland-McGill 精度 / Mackinlay APT。
...
## 1b. Mackinlay 有效性排名(从最准到最不准,底部 = 对该类型不该用)

| 数据类型 | 排序(高→低) |
|---|---|
| 定量 Quantitative | position → length → angle → slope → area → volume → density(明度) → saturation → hue → texture → connection → containment → shape |
| 有序 Ordinal | position → density → saturation → hue → texture → connection → containment → length → angle → slope → area → volume → shape |
| 名义 Nominal | position → hue → texture → connection → containment → density → saturation → shape → length → angle → slope → area → volume |
...
## 3. expressiveness / effectiveness → encode-check 的规则

**expressiveness(恰好编码全部且仅有事实):**
- 名义数据排上位置/长度轴(凭空造顺序)→ **error**
- 截断轴(bar 不从 0)→ **error**(graphical integrity,chart faithfulness lint 管)
- **彩虹色 = 色相在变却不编码任何类别 → 凭空编码了不存在的「类别事实」** → `encode-check` color-budget(≤4)
- 关系画了却不 binding / 关系没给 connection(只靠邻近)→ `encode-check` relation-encoded(warn)

**effectiveness(有更准的没用):**
- 数量用了 area/hue,而 length/position 可用 → 退档(chart faithfulness)
- 有序用 hue(无序通道)而非 value/size → 退档
- 主角没拿到最强通道组合(size/hue)/ 强调不唯一 → `encode-check` hero-unique;深档比 hero 还重 → emphasis-order

落点:前两条进 chart faithfulness lint;彩虹/预算/主角/关系/分组进 `scripts/encode-check.mjs`;几何残差(浮空/穿框/绕背)仍由 `arch-lint` / `floor-check` 管。

## 4. 在流水线里的位置(把它变成回路)

```
view.ir(选角:hero/tiers/groups/density)
  → encode.ir:给每个 item/relation/group 标数据类型 → 查「类型 → Excalidraw 通道」表(§1+§2)→ 选 mark + 通道值(忠实集内最优,= Mackinlay effectiveness)
  → encode-check(expressiveness/effectiveness 残差,§3)
  → layout(定坐标)→ render(.excalidraw)→ arch-lint/floor-check(几何残差)
```
