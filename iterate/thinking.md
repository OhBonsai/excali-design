...
**1) Bertin —— 不只是「有哪些视觉变量」,关键是「每个变量能编码什么」**

marks 三类(implantation):**点 / 线 / 面**。变量 = 2 个平面维(x,y 位置) + 6 个视网膜变量:大小、明度、纹理、色相、方向、形状。
但 Bertin 真正的贡献是给每个变量标了 **4 种知觉属性**——这决定了「什么数据只能用什么变量」:

| 变量 | 选择性(一眼挑出同类) | 关联性(不抢戏,可视为同级) | 有序性(看得出顺序) | 定量性(读得出比例) |
|---|---|---|---|---|
| 位置 | ✓ | ✓ | ✓ | ✓ |
| 大小 | ✓ | ✗(重的抢戏) | ✓ | ✓ |
| 明度 | ✓ | ✗ | ✓ | ✗ |
...
直接读出规则:**定量数据 → 位置/大小;有序 → 明度/大小;名义(类别)→ 色相/形状/纹理。** 反过来——用色相去编「数量」(颜色深浅当数值)、用大小去编「类别」,都是属性错配,看不准。

**三层阅读(levels of reading)**:元素级(看单个 mark 的值)/ 中间级(看一组/子集的趋势)/ 整体级(一眼看全图拿一个结论)。Bertin 的效率判据:**一张好图回答问题所需的「心理图像」越少越好,理想 = 1(一眼)**——这就是「眯眼测试」的学术祖宗,也对应 §1.3 的「一条主脊 + 整体先读出」。

**2) Cleveland & McGill 1984 —— 把「看得准」做成了实验排序**

不是拍脑袋,是测人判断误差排出来的**初级知觉任务精度榜**(准 → 不准):

> 共同标尺上的位置 > 非对齐标尺上的位置 > 长度 / 方向 / 角度 > 面积 > 体积 / 曲率 > 明暗(色值)/ 饱和度

工程含义:**最重要的定量比较,编进位置/长度;躲开面积(气泡)、角度(饼)、颜色深浅(热力)去表达精确数量。** 因为是实测,它**够格当 lint 的地板**(违反=可量化的「看不准」)。

**3) Mackinlay 1986 APT —— 从「排序」到「自动选图」,你引擎的直接祖宗**

APT(A Presentation Tool)把选图变成可搜索的合成问题,两条判据:

- **Expressiveness(表达得准)**:所选图形语言要恰好编码**全部且仅有**这些事实——不多不少。典型违反:**用「位置/长度」这种有序量化变量去画名义类别 → 凭空制造了不存在的顺序/数量**(把「红/绿/蓝」摆成一条轴,暗示它们有大小);或截断轴 → 暗示了不存在的零点。这是**诚实性**判据,先于美观。
- **Effectiveness(看得准)**:在「表达得准」的候选里,挑知觉上最易读的——用排序操作化。

而且 Mackinlay **把 Cleveland-McGill 的单一排序拆成「按数据类型各一份」**(这是最关键的升级):

| 数据类型 | 编码偏好(高 → 低,取前列) |
|---|---|
| 定量 quantitative | 位置 > 长度 > 角度/斜率 > 面积 > 体积 > 浓度/饱和 > **色相** > 纹理 > 形状 |
| 有序 ordinal | 位置 > 明度/浓度 > 饱和 > 色相 > 纹理 > … |
| 名义 nominal | 位置 > **色相** > 纹理 > 形状 > … > (长度/角度/面积 垫底) |

注意那个**反转**:色相对「定量」几乎垫底,对「名义」却仅次于位置。所以「用什么编码」不能脱离「数据是什么类型」——这正是 dispatch 要先读数据类型的原因。APT 的合成回路(**先 expressive 筛掉错配 → 再按 effectiveness 排序挑最优 → 渲染**)就是后来一切自动制图工具(Tableau 的 Show Me 直接源于此)的母版。

**→ 落到引擎(§6.5 / §9.2 的理论根)**

- **compute(选编码)**:`数据类型(定/序/名)→ 上面的偏好表 → 自动选 mark + 变量`。这就是「能算对就别检查」里「选编码」那一步的算法依据。
- **lint(查编码)**:① expressiveness 违反——名义数据被排上有序/量化轴、截断轴、面积/角度表达精确量 → 报 error(这是 §9.2 chart faithfulness lint 的学名);② effectiveness 退档——有更准的编码却用了更弱的(数量用了面积而长度可用)→ 报 warn。
- **Bertin 属性矩阵**就是那张查表本身;**Cleveland-McGill** 给「地板该卡哪」;**Mackinlay** 给「整条自动选图回路 + 按类型分」。三者合起来 = 把「画师凭手感选编码」形式化成可计算 + 可检查。
- **Goodhart 边界**:排序测的是「初级精度」,不是全部。部分整体且 ≤4 类时饼图可接受、地图上面积/位置本就是地理量、gestalt/审美有时值得用次优编码换整体感。**当地板,不当铁律**(同 §6.6)。

> **落成规范**:这套理论已对接 Excalidraw 真实 d.ts,逐变量映射到元素旋钮 + 按类型选编码 + expressiveness/effectiveness 当 lint,见 `references/encoding.md`(关键结论:Excalidraw 的强通道是 connection/containment,故为关系图工具)。
