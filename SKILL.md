...
3. Answer the "design pre-questions" before deciding the system. These decide success more than any styling detail; quality is set before you draw.
   ...
   - **MANDATORY GATE (free-form info-viz): write the files before you draw.** For any free-form information visualization (NOT prototypes / fixed-Mermaid, whose data is given), you MUST produce these artifacts and pass their checks, and only then create the `.excalidraw`:
     ...
     The `.excalidraw` is rendered from the **view.ir**, not the data.ir. Do NOT skip these and "do it in your head" — the files are the contract + the cut/split record. Worked example: `examples/self-arch/` (`brief.json` → `data-ir.json` → `overview.view-ir.json` → `overview.excalidraw`).
...
- Information before pixels (MANDATORY for free-form info-viz): no `.excalidraw` until `<name>.brief.json` → `<name>.data-ir.json` → `<name>.view-ir.json` (one per diagram) all exist and pass their checks (`data-ir-check` 0 error; `--view --from` 0 error). The `.excalidraw` is rendered from the view.ir. Writing them "in your head" is not allowed — they are the contract + cut/split record. Templates `examples/data-ir/_TEMPLATE.*`; LLM step prompt `prompts/view-ir.md`; worked example `examples/self-arch/`.
