...
3. Answer the "design pre-questions" before deciding the system. These decide success more than any styling detail; quality is set before you draw.
   ...
   - **MANDATORY GATE (free-form info-viz): write the files before you draw.** For any free-form information visualization (NOT prototypes / fixed-Mermaid, whose data is given), you MUST produce these artifacts and pass their checks, and only then create the `.excalidraw`:
       ...
       2. `<name>.data-ir.json` — the data.ir (message / dataset_type / items / attributes / relations / salience / scope / budget). **Schema authority = `references/data-ir.d.ts`**. Templates: `examples/data-ir/_TEMPLATE.data-ir.{json,ts}`. Run `node scripts/data-ir-check.mjs <name>.data-ir.json` → must be **0 error** (fix/split per its advice).
       3. `<name>.<diagram>.view-ir.json` — **one per diagram in `data.ir.scope.diagrams`**: project the data.ir onto that diagram, then do editorial subtraction (ikebana: cut to the essence, active whitespace) + casting (one `hero`, ordered `tiers`). This step is LLM-driven — use the prompt `prompts/view-ir.md`. **Schema authority = `references/view-ir.d.ts`**. Validate: `node scripts/data-ir-check.mjs <name>.view-ir.json --view --from <name>.data-ir.json` → must be **0 error** (`--from` enforces items ⊆ the diagram's covers — the view may not invent data).
       4. `<name>.<diagram>.encode-ir.json` — bind each item/relation/group to **honest visual channels** (mark/shape, hue, value/size/weight, arrowhead/stroke, containment) — **position-free** (coordinates are layout's job). **Half-automatic**: the mechanical channels are a deterministic function of the view.ir, so the **program** fills them — `node scripts/encode-derive.mjs <name>.view-ir.json --overrides <name>.encode-overrides.json --out <name>.encode-ir.json` (tier→size/weight/value gradient, hero→accent, flow/dependency→solid/dashed, group→containment). The **LLM** only writes a small `<name>.encode-overrides.json` with the genuinely semantic calls (which shape/icon per role, special UML/ER arrowheads, nominal `cat:` hues) — prompt `references/encoding.md`. **Schema authority = `references/encode-ir.d.ts`**. Validate: `node scripts/encode-check.mjs <name>.encode-ir.json --from <name>.view-ir.json` → must be **0 error** (enforces marks==items, every relation a connection, every group a containment, hero/accent unique, color budget ≤4).
     The `.excalidraw` is rendered from the **view.ir + encode.ir**, not the data.ir. Do NOT skip these and "do it in your head" — the files are the contract + the cut/split + encoding record. Worked example: `examples/self-arch/` (`brief.json` → `data-ir.json` → `overview.view-ir.json` → `overview.encode-ir.json` → `overview.excalidraw`).
...
| `encode.ir` — the encoding layer as a formal artifact: bind each item/relation/group to honest visual channels (mark/hue/value/size/weight/arrowhead/stroke/containment), **position-free**. LLM step driven by the prompt `references/encoding.md` (data-type→channel, Bertin/Cleveland-McGill/Mackinlay; Excalidraw's strong channels = connection + containment → relational tool, not stats-chart tool). Schema `references/encode-ir.d.ts`; check `scripts/encode-check.mjs <encode.ir.json> --from <view.ir.json>`. Theory record: `iterate/encoding-design.md` | `references/encoding.md` + `references/encode-ir.d.ts` |
...
| `view.ir` — per-diagram curated artifact (editorial subtraction + casting) between data.ir and rendering. Produce via LLM prompt `prompts/view-ir.md`; schema `references/view-ir.d.ts`; check `scripts/data-ir-check.mjs <view.ir.json> --view --from <data-ir.json>`. The `.excalidraw` is rendered from this, not the data.ir | `references/view-ir.d.ts` + `prompts/view-ir.md` |
| `encode.ir` — channel-binding artifact between view.ir and rendering (mark/hue/value/size/weight/arrowhead/stroke/containment, **position-free**). Produce via LLM prompt `references/encoding.md`; schema `references/encode-ir.d.ts`; check `scripts/encode-check.mjs <encode.ir.json> --from <view.ir.json>`. Enforces honest encoding (hero/accent unique, every relation→connection, every group→containment, color ≤4) | `references/encode-ir.d.ts` + `references/encoding.md` |
...
- Information before pixels (MANDATORY for free-form info-viz): no `.excalidraw` until `<name>.brief.json` → `<name>.data-ir.json` → `<name>.view-ir.json` → `<name>.encode-ir.json` (view + encode one per diagram) all exist and pass their checks (`data-ir-check` 0 error; `--view --from` 0 error; `encode-check --from` 0 error). The `.excalidraw` is rendered from view.ir + encode.ir. Writing them "in your head" is not allowed — they are the contract + cut/split + encoding record. Templates `examples/data-ir/_TEMPLATE.*`; LLM prompts `prompts/view-ir.md` + `references/encoding.md`; worked example `examples/self-arch/`.
